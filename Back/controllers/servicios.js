...
const agregarServicioClienteMoto = async(req, res) => {
    try {
        ...
        const nuevaMoto = await Moto.findById(servicio.moto).populate("cliente");
        if (!nuevaMoto || !nuevaMoto.cliente) {
            return res.status(404).json({
                ok: false,
                msg: "No se encontró la moto o el cliente asociado."
            });
        }
        ...
        return res.json({
            ok: true,
            msg: "El servicio se agregó correctamente",
            servicio: nuevoServicio._id,
            clienteNombre: `${nuevaMoto.cliente.nombres} ${nuevaMoto.cliente.apellidos}`
        });
